...
class FC_API_Admin {
	...
	private function __construct(){
		...
		add_action('woocommerce_process_product_meta', [
			$this,
			'fc_api_woocommerce_product_key_fields_save'
		]);
				...
				if(count($variations)>0){
					foreach($variations as $variation){
						if($variation['variation_is_active'] && $variation['variation_is_visible']){
							...
							$variation_sale_price = $variation_object->sale_price;
							$variation_discount = [];
							...
							if($variation_sale_price && $variation_sale_price<$variation_regular_price){
								$variation_date_on_sale_from = $variation_object->date_on_sale_from;	
								$variation_date_on_sale_to = $variation_object->date_on_sale_to;
