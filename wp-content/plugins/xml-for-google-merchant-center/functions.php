...
function xfgmc_optionGET($option_name, $n = '', $type = '', $source_settings_name = '') {
	...
	if (defined('xfgmcp_VER')) {$pro_ver_number = xfgmcp_VER;} else {$pro_ver_number = '2.2.7';}
	...
	switch ($type) {
		...
		default:
			...
			if ($option_name === 'xfgmc_desc') {return xfgmc_optionGET($option_name, $n, 'set_arr');}		
			if ($option_name === 'xfgmc_no_default_png_products') {return xfgmc_optionGET($option_name, $n, 'set_arr');}
			if ($option_name === 'xfgmc_whot_export') {return xfgmc_optionGET($option_name, $n, 'set_arr');}
			if ($option_name === 'xfgmc_feed_assignment') {return xfgmc_optionGET($option_name, $n, 'set_arr');}
			/* for old premium versions */
			if ($n === '1') {$n = '';}
			$option_name = $option_name.$n;
			if (is_multisite()) { 
