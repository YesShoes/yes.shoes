	 ...
	 <div class="postbox">
	  ...
	  <div class="inside">		
		<form action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" enctype="multipart/form-data">
		 ...
		 if (isset($_POST['xfgmc_num_feed'])) {$xfgc_num_feed = sanitize_text_field($_POST['xfgmc_num_feed']);} else {$xfgc_num_feed = '1';} 
			...
			<td class="overalldesc">
				<select style="width: 100%" name="xfgmc_num_feed" id="xfgmc_num_feed">
					...
					for ($i = 0; $i < count($xfgmc_settings_arr_keys_arr); $i++) :
						...
						<option value="<?php echo $numFeed; ?>" <?php selected($xfgc_num_feed, $numFeed); ?>><?php _e('Feed', 'xfgmc'); ?> <?php echo $numFeed; ?>: feed-xml-<?php echo $cur_blog_id; ?>.xml<?php echo $feed_assignment; ?></option>
					<?php endfor; ?>
				</select>
			</td>
		 </tr>	
