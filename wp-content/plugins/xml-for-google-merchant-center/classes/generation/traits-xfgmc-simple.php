...
trait XFGMC_Trait_Simple_Get_Id {
	public function get_id() {
		...
		$result_xml_id = apply_filters('xfgmc_simple_result_xml_id', $result_xml_id, $product, $xfgmc_instead_of_id, $this->get_feed_id());
		// $result_xml_id = apply_filters('xfgmc_simple_result_xml_id', $result_xml_id, $xfgmc_instead_of_id, $this->get_input_data_arr());
...
trait XFGMC_Trait_Simple_Get_Name {
	public function get_name() {
		...
		$result_xml_name = apply_filters('xfgmc_change_name', $result_xml_name, $product->get_id(), $product, $this->get_feed_id());
...
trait XFGMC_Trait_Simple_Get_Description {
	public function get_description() {
		...
		$result_xml_desc = '';
		$description_xml = apply_filters('xfgmc_description_xml_filter', $description_xml, $product->get_id(), $product, $this->get_feed_id()); /* с версии 2.2.1 */
		if (!empty($description_xml)) {
			...
			$description_xml = apply_filters('xfgmc_description_filter', $description_xml, $product->get_id(), $product, $this->get_feed_id());
...
trait XFGMC_Trait_Simple_Get_Google_Product_Category {
	public function get_google_product_category() {
		$product = $this->get_product();
//		$site_category_id_arr = wc_get_product_cat_ids($product->get_id());
//		$site_category_id = $site_category_id_arr[0];

		if (get_post_meta($product->get_id(), 'xfgmc_google_product_category', true) == '') {
			...
			$xfgmc_google_product_category = get_post_meta($product->get_id(), 'xfgmc_google_product_category', true);
			$xfgmc_google_product_category = htmlspecialchars($xfgmc_google_product_category);			
			$result_xml_category = new XFGMC_Get_Paired_Tag('g:google_product_category', $xfgmc_google_product_category);
		}
...
//		$site_category_id = $site_category_id_arr[0];
		...
		if (get_post_meta($product->get_id(), '_xfgmc_fb_product_category', true) == '') {
			...
			$xfgmc_fb_product_category = get_post_meta($product->get_id(), '_xfgmc_fb_product_category', true);
			$xfgmc_fb_product_category = htmlspecialchars($xfgmc_fb_product_category);
			$result_xml_fb_product_category = new XFGMC_Get_Paired_Tag('g:fb_product_category', $xfgmc_fb_product_category);
		}
...
//		$site_category_id = $site_category_id_arr[0];
		...
		if (get_post_meta($product->get_id(), '_xfgmc_tax_category', true) == '') {
			...
			$xfgmc_tax_category = get_post_meta($product->get_id(), '_xfgmc_tax_category', true);
			$xfgmc_tax_category = htmlspecialchars($xfgmc_tax_category);
			$result_xml_xfgmc_tax_category = new XFGMC_Get_Paired_Tag('g:tax_category', $xfgmc_tax_category);
		}
...
//		$site_category_id = $site_category_id_arr[0];
		...
		if ($xfgmc_product_type === 'enabled') {
			...
			$product_type_res = apply_filters('xfgmc_product_type_res_simple_filter', $product_type_res, $this->get_cat_id(), $product, $this->get_feed_id());
...
//		$site_category_id = $site_category_id_arr[0];
		...
		$result_url = apply_filters('xfgmc_url_filter_simple', $result_url, $product, $this->get_cat_id(), $this->get_feed_id()); /* с версии 2.0.5 */
...
trait XFGMC_Trait_Simple_Get_Image_Link {
	public function get_image_link() {
		...
		$picture_xml = apply_filters('xfgmc_pic_simple_offer_filter', $picture_xml, $product, $this->get_feed_id());
...
trait XFGMC_Trait_Simple_Get_Identifier_Exists {
	public function get_identifier_exists() {
		$product = $this->get_product();

		if ((get_post_meta($product->get_id(), 'xfgmc_identifier_exists', true) !== '') && (get_post_meta($product->get_id(), 'xfgmc_identifier_exists', true) !== 'off')) {
			$identifier_exists_val = get_post_meta($product->get_id(), 'xfgmc_identifier_exists', true);
			$result_xml_identifier_exists = new XFGMC_Get_Paired_Tag('g:identifier_exists', $identifier_exists_val); 
			if ($identifier_exists_val === 'no') {
				$result_xml_identifier_exists .= "<g:gtin></g:gtin>".PHP_EOL;
				$result_xml_identifier_exists .= "<g:mpn></g:mpn>".PHP_EOL;
			}
		} else {$result_xml_identifier_exists = '';}

		return $result_xml_identifier_exists = '';
	}
}
...
trait XFGMC_Trait_Simple_Get_Adult {
	public function get_adult() {
		...
		if ((get_post_meta($product->get_id(), 'xfgmc_adult', true) !== '') && (get_post_meta($product->get_id(), 'xfgmc_adult', true) !== 'off')) {
			$xfgmc_adult = get_post_meta($product->get_id(), 'xfgmc_adult', true);
			$result_xml_adult = new XFGMC_Get_Paired_Tag('g:adult', $xfgmc_adult); 
		} else {$result_xml_adult = '';} 
...
trait XFGMC_Trait_Simple_Get_Is_Bundle {
	public function get_is_bundle() {
		...
		if ((get_post_meta($product->get_id(), '_xfgmc_is_bundle', true) === 'yes') || (get_post_meta($product->get_id(), '_xfgmc_is_bundle', true) === 'no')) {
			$xfgmc_is_bundle = get_post_meta($product->get_id(), '_xfgmc_is_bundle', true);
			$result_xml_is_bundle = new XFGMC_Get_Paired_Tag('g:is_bundle', $xfgmc_is_bundle); 
		} else {$result_xml_is_bundle = '';}
...
trait XFGMC_Trait_Simple_Get_Multipack {
	public function get_multipack() {
		...
		if (get_post_meta($product->get_id(), '_xfgmc_multipack', true) !== '') {
			$xfgmc_multipack = get_post_meta($product->get_id(), '_xfgmc_multipack', true);
			$result_xml_multipack = new XFGMC_Get_Paired_Tag('g:multipack', $xfgmc_multipack); 
		} else {$result_xml_multipack = '';}
...
trait XFGMC_Trait_Simple_Get_Condition {
	public function get_condition() {
		...
		$xfgmc_condition = get_post_meta($product->get_id(), 'xfgmc_condition', true);
		switch ($xfgmc_condition) { 
			...
			case 'default': 
				...
				$result_xml_condition = new XFGMC_Get_Paired_Tag('g:condition', $xfgmc_default_condition);
			break;
			default: $result_xml_condition = new XFGMC_Get_Paired_Tag('g:condition', $xfgmc_condition);
		}
...
trait XFGMC_Trait_Simple_Get_Custom_Label {
	public function get_custom_label() {
		...
		$result_custom_label = '';
		if (get_post_meta($product->get_id(), 'xfgmc_custom_label_0', true) !== '') {
			$xfgmc_custom_label_0 = get_post_meta($product->get_id(), 'xfgmc_custom_label_0', true);	
			$result_custom_label .= '<g:custom_label_0>'.$xfgmc_custom_label_0.'</g:custom_label_0>'.PHP_EOL;
		}
		if (get_post_meta($product->get_id(), 'xfgmc_custom_label_1', true) !== '') {
			$xfgmc_custom_label_1 = get_post_meta($product->get_id(), 'xfgmc_custom_label_1', true);	
			$result_custom_label .= '<g:custom_label_1>'.$xfgmc_custom_label_1.'</g:custom_label_1>'.PHP_EOL;
		}
		if (get_post_meta($product->get_id(), 'xfgmc_custom_label_2', true) !== '') {
			$xfgmc_custom_label_2 = get_post_meta($product->get_id(), 'xfgmc_custom_label_2', true);	
			$result_custom_label .= '<g:custom_label_2>'.$xfgmc_custom_label_2.'</g:custom_label_2>'.PHP_EOL;
		}
		if (get_post_meta($product->get_id(), 'xfgmc_custom_label_3', true) !== '') {
			$xfgmc_custom_label_3 = get_post_meta($product->get_id(), 'xfgmc_custom_label_3', true);	
			$result_custom_label .= '<g:custom_label_3>'.$xfgmc_custom_label_3.'</g:custom_label_3>'.PHP_EOL;
		}
		if (get_post_meta($product->get_id(), 'xfgmc_custom_label_4', true) !== '') {
			$xfgmc_custom_label_4 = get_post_meta($product->get_id(), 'xfgmc_custom_label_4', true);	
...
trait XFGMC_Trait_Simple_Get_Price {
	public function get_price() {
		...
		if ($price_xml == 0 || empty($price_xml)) { 
			$this->add_skip_reason(array('reason' => __('Price not specified', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
		}
			...
			if ((xfgmc_optionGET('xfgmcp_compare_value', $this->get_feed_id(), 'set_arr') !== false) && (xfgmc_optionGET('xfgmcp_compare_value', $this->get_feed_id(), 'set_arr') !== '')) {
				...
				if ($xfgmcp_compare == '>=') {
					if ($price_xml < $xfgmcp_compare_value) {
						$this->add_skip_reason(array('reason' => __('The product price', 'xfgmc').' '. $product->get_price(). ': < ' . $xfgmcp_compare_value, 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
					}
				} else {
					if ($price_xml >= $xfgmcp_compare_value) {
						$this->add_skip_reason(array('reason' => __('The product price', 'xfgmc').' '. $product->get_price(). ': >= ' . $xfgmcp_compare_value, 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
		...
		$currencyId_xml = apply_filters('xfgmc_change_price_currency', $currencyId_xml, $product, $this->get_feed_id()); /* с версии 2.2.10 */

		$price_xml = apply_filters('xfgmc_simple_price_xml_filter', $price_xml, $product, $this->get_feed_id()); /* с версии 2.0.6 */
		$xfgmc_sale_price = xfgmc_optionGET('xfgmc_sale_price', $this->get_feed_id(), 'set_arr');
		if ($xfgmc_sale_price === 'yes') {
			$sale_price = (float)$product->get_sale_price();
			$sale_price = apply_filters('xfgmc_simple_sale_price_xml_filter', $sale_price, $product, $this->get_feed_id()); /* с версии 2.3.1 */
			...
			if (($sale_price > 0) && ($price_xml === $sale_price)) {
				...
				$result_xml = apply_filters('xfgmc_simple_sale_price_filter', $result_xml, $product, $this->get_feed_id()); /* с версии 2.0.6 */
			} else {
...
trait XFGMC_Trait_Simple_Get_Pricing_Measure {
	public function get_unit_pricing_measure() {
		...
		if (get_post_meta($product->get_id(), '_xfgmc_unit_pricing_measure', true) !== '') {
			$unit_pricing_measure_xml = get_post_meta($product->get_id(), '_xfgmc_unit_pricing_measure', true);
			$result_xml_unit_pricing_measure = new XFGMC_Get_Paired_Tag('g:unit_pricing_measure', $unit_pricing_measure_xml);
		} 
		if (class_exists('WooCommerce_Germanized')) {
			if (!empty(wc_gzd_get_gzd_product($product->get_id())->get_unit())) {
				$unit_germanized = ' '.wc_gzd_get_gzd_product($product->get_id())->get_unit();
			};

			if (!empty(wc_gzd_get_gzd_product($product->get_id())->get_unit_product())) {
				$result_xml_unit_pricing_measure = "<g:unit_pricing_measure>".wc_gzd_get_gzd_product($product->get_id())->get_unit_product().$unit_germanized."</g:unit_pricing_measure>".PHP_EOL;
			}; 
		}
...
trait XFGMC_Trait_Simple_Get_Base_Measure {
	public function get_unit_pricing_base_measure() {
		...
		if (get_post_meta($product->get_id(), '_xfgmc_unit_pricing_base_measure', true) !== '') {					
			$unit_pricing_base_measure_xml = get_post_meta($product->get_id(), '_xfgmc_unit_pricing_base_measure', true);
		...
		if (class_exists('WooCommerce_Germanized')) {
			...
			if (!empty(wc_gzd_get_gzd_product($product->get_id())->get_unit_base())) {
				$result_xml_pricing_base_measure = "<g:unit_pricing_base_measure>".wc_gzd_get_gzd_product($product->get_id())->get_unit_base().$unit_germanized."</g:unit_pricing_base_measure>".PHP_EOL;
			};
		}
...
trait XFGMC_Trait_Simple_Get_Return_Rule_Label {
	public function get_return_rule_label() {
		...
		if (get_post_meta($product->get_id(), '_xfgmc_return_rule_label', true) !== '') {
			$xfgmc_return_rule_label = get_post_meta($product->get_id(), '_xfgmc_return_rule_label', true);	
			...
			switch ($xfgmc_s_return_rule_label) { /* disabled, default_value, post_meta */
				...
				case "post_meta":
					...
					if (get_post_meta($product->get_id(), $xfgmc_return_rule_label_id, true) !== '') {
						$result_return_rule_label_xml = get_post_meta($product->get_id(), $xfgmc_return_rule_label_id, true);
...
trait XFGMC_Trait_Simple_Get_Store_Code {
	public function get_store_code() {
		...
		if (get_post_meta($product->get_id(), '_xfgmc_store_code', true) !== '') {
			$xfgmc_store_code = get_post_meta($product->get_id(), '_xfgmc_store_code', true);	
...
trait XFGMC_Trait_Simple_Get_Gtin {
	public function get_gtin() {
		...
		$xfgmc_identifier_exists = get_post_meta($product->get_id(), 'xfgmc_identifier_exists', true);
		...
		switch ($gtin) { /* disabled, sku, post_meta или id */
			...
			case "post_meta":
				...
				if (get_post_meta($product->get_id(), $gtin_post_meta_id, true) !== '') {
					$gtin_xml = get_post_meta($product->get_id(), $gtin_post_meta_id, true);
			...
			case "germanized":
				if (class_exists('WooCommerce_Germanized')) {
					if (get_post_meta($product->get_id(), '_ts_gtin', true) !== '') {
						$gtin_xml = get_post_meta($product->get_id(), '_ts_gtin', true);
						$result_xml .= "<g:gtin>".$gtin_xml."</g:gtin>".PHP_EOL;
					}
				}
			break;			
	...
	public function get_mpn() {
		...
		switch ($mpn) { /* disabled, sku, post_meta или id */
			...
			case "post_meta":
				...
				if (get_post_meta($product->get_id(), $mpn_post_meta_id, true) !== '') {
					$mpn_xml = get_post_meta($product->get_id(), $mpn_post_meta_id, true);
					$result_xml .= "<g:mpn>".htmlspecialchars($mpn_xml)."</g:mpn>".PHP_EOL;
				}
			break;	
			case "germanized":
				if (class_exists('WooCommerce_Germanized')) {
					if (get_post_meta($product->get_id(), '_ts_mpn', true) !== '') {
						$mpn_xml = get_post_meta($product->get_id(), '_ts_mpn', true);
						$result_xml .= "<g:mpn>".htmlspecialchars($mpn_xml)."</g:mpn>".PHP_EOL;
					}
				}
			break;
	...
	public function get_age_group() {
		...
		switch ($age) { /* disabled, sku, post_meta или id */
			...
			case "post_meta":
				...
				if (get_post_meta($product->get_id(), $age_post_meta_id, true) !== '') {
					$age_xml = get_post_meta($product->get_id(), $age_post_meta_id, true);
	...
	public function get_brand() {
		...
		if (!empty($brand) && $brand !== 'off') {	 
			if ((is_plugin_active('perfect-woocommerce-brands/perfect-woocommerce-brands.php') || is_plugin_active('perfect-woocommerce-brands/main.php') || class_exists('Perfect_Woocommerce_Brands')) && $brand === 'sfpwb') {
				...
				if (get_post_meta($product->get_id(), $brand_post_meta_id, true) !== '') {
					$brand_xml = get_post_meta($product->get_id(), $brand_post_meta_id, true);
...
trait XFGMC_Trait_Simple_Get_Size_Type {
	public function get_size_type() {
		...
		// тип размера
		if (get_term_meta($this->get_cat_id(), 'xfgmc_size_type', true) == '' || get_term_meta($this->get_cat_id(), 'xfgmc_size_type', true) === 'default') {
...
trait XFGMC_Trait_Simple_Get_Shipping_Xml {
	public function get_shipping_xml() {
		...
		if (get_post_meta($product->get_id(), '_xfgmc_min_handling_time', true) !== '') {
			$xfgmc_min_handling_time = get_post_meta($product->get_id(), '_xfgmc_min_handling_time', true);	
		...
		if (get_post_meta($product->get_id(), '_xfgmc_max_handling_time', true) !== '') {
			$xfgmc_max_handling_time = get_post_meta($product->get_id(), '_xfgmc_max_handling_time', true);	
		...
		if (get_post_meta($product->get_id(), '_xfgmc_shipping_label', true) !== '') {
		   $xfgmc_shipping_label = get_post_meta($product->get_id(), '_xfgmc_shipping_label', true);	
		...
		if (!empty($weight_xml)) {
			...
			$xfgmc_def_shipping_weight_unit = apply_filters('xfgmc_simple_def_shipping_weight_unit_filter', $xfgmc_def_shipping_weight_unit, $product, $this->get_feed_id()); /* с версии 2.4.0 */
		...
		if (class_exists('WPSEO_Primary_Term')) {		  
			...
			if ($catidWPSEO !== false) { 
				...
				if ($termini !== false) {
					...
					new XFGMC_Error_Log('FEED № '.$this->get_feed_id().'; WARNING: Для товара $product->get_id() = '.$product->get_id().' get_the_terms = false. Возможно база битая. Пробуем задействовать wp_get_post_terms; Файл: offer.php; Строка: '.__LINE__);
					...
					if (is_array($product_cats) && count($product_cats)) {
						...
						new XFGMC_Error_Log('FEED № '.$this->get_feed_id().'; WARNING: Для товара $product->get_id() = '.$product->get_id().' база наверняка битая. wp_get_post_terms вернула массив. $catid = '.$catid.'; Файл: offer.php; Строка: '.__LINE__);
					}
				}
			}
...
trait XFGMC_Trait_Skips {
	public function get_skips() {
		...
		if ($product == null) {
			$this->add_skip_reason(array('reason' => __('There is no product with this ID', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
		}

		if ($product->is_type('grouped')) {
			$this->add_skip_reason(array('reason' => __('Product is grouped', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
		 }
		...
		if ($product->is_type('variable')) {	
			if ($xfgmc_whot_export === 'simple') {
				$this->add_skip_reason(array('reason' => __('Product is simple', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return ''; 
			}
		}
		if ($product->is_type('simple')) {	
			if ($xfgmc_whot_export === 'variable') {
				$this->add_skip_reason(array('reason' => __('Product is variable', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
			}
		}
		
		$special_data_for_flag = '';
		$special_data_for_flag = apply_filters('xfgmc_special_data_for_flag_filter', $special_data_for_flag, $product, $this->get_feed_id()); /* с версии 2.2.7 */ 
		
		$skip_flag = apply_filters('xfgmc_skip_flag', $skip_flag, $product->get_id(), $product, $special_data_for_flag, $this->get_feed_id()); /* c версии 2.2.0, с версии 2.2.7 добавелн $special_data_for_flag */
		if ($skip_flag === true) {
			$this->add_skip_reason(array('reason' => __('Flag', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
		}
		...
		if ($xfgmc_skip_missing_products == 'on') {
			if ($product->is_in_stock() == false) { 
				$this->add_skip_reason(array('reason' => __('Skip missing products', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
			}
		}
		...
		if ($skip_backorders_products == 'on') {
			if ($product->get_manage_stock() == true) { // включено управление запасом  
				if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) { 
					$this->add_skip_reason(array('reason' => __('Skip backorders products', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
				}
			} else {
				if ($product->get_stock_status() !== 'instock') { 
					$this->add_skip_reason(array('reason' => __('Skip backorders products', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-simple.php', 'line' => __LINE__)); return '';
		...
		if ($product->is_type('variable')) {	
			...
			if ($xfgmc_skip_missing_products == 'on') {
				if ($offer->is_in_stock() == false) { 
					$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('Skip missing products', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-variable.php', 'line' => __LINE__)); return '';
				}
			}
			...
			if ($skip_backorders_products == 'on') {
				if ($offer->get_manage_stock() == true) { // включено управление запасом			  
					if (($offer->get_stock_quantity() < 1) && ($offer->get_backorders() !== 'no')) {
						$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('Skip backorders products', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-variable.php', 'line' => __LINE__)); return '';
					}
				}
			}
	
			$skip_flag = apply_filters('xfgmc_skip_flag_variable', $skip_flag, $product->get_id(), $product, $offer, $special_data_for_flag, $this->get_feed_id()); /* c версии 2.2.0, с версии 2.2.7 добавелн $special_data_for_flag */
			if ($skip_flag === true) {
				$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('Flag', 'xfgmc'), 'post_id' => $product->get_id(), 'file' => 'traits-xfgmc-variable.php', 'line' => __LINE__)); return '';
			}
		}
	}
