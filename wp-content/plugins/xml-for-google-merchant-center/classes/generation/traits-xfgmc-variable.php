...
trait XFGMC_Trait_Variable_Get_Id {
	public function get_id() {
		...
		// $result_xml_id = apply_filters('xfgmc_variable_result_xml_id', $result_xml_id, $xfgmc_instead_of_id, $this->get_input_data_arr());

		$result_xml_id = apply_filters('xfgmc_variable_result_xml_id', $result_xml_id, $product, $offer, $xfgmc_instead_of_id, $this->get_feed_id()); /* c версии 2.3.12 */
...
trait XFGMC_Trait_Variable_Get_Name {
	public function get_name() {
		$product = $this->product;
		$offer = $this->offer;
		$result_xml_name = $offer->get_title();
//		$result_xml_name = apply_filters('xfgmc_variable_change_name', $result_xml_name, $this->get_input_data_arr());

		$result_xml_name = apply_filters('xfgmc_variable_change_name', $result_xml_name, $product->get_id(), $product, $offer, $this->get_feed_id());
...
trait XFGMC_Trait_Variable_Get_Description {
	public function get_description() {
		...
		if (!empty($description_xml)) {
			...
			$description_xml = xfgmc_max_lim_text($description_xml, 5000);			
			$description_xml = apply_filters('xfgmc_description_filter', $description_xml, $product->get_id(), $product, $this->get_feed_id());
			$description_xml = apply_filters('xfgmc_description_filter_variable', $description_xml, $product->get_id(), $product, $offer, $this->get_feed_id()); /* с версии 2.2.1 */			
			$description_xml = trim($description_xml);
			if ($description_xml !== '') {
...
//		$site_category_id = $site_category_id_arr[0]; // int или array
		...
		$result_xml_category = apply_filters('xfgmc_xml_google_cat_variable_filter', $result_xml_category, $this->get_cat_id(), $product, $offer, $this->get_feed_id());	
//		$result_xml_category = apply_filters('xfgmc_xml_google_cat_variable_filter', $result_xml_category, $this->get_cat_id(), $this->get_input_data_arr());
...
//		$site_category_id = $site_category_id_arr[0];
		...
		$result_xml_fb_product_category = apply_filters('xfgmc_xml_facebook_cat_variable_filter', $result_xml_fb_product_category, $this->get_cat_id(), $product, $offer, $this->get_feed_id());	
		// $result_xml_fb_product_category = apply_filters('xfgmc_xml_facebook_cat_variable_filter', $result_xml_fb_product_category, $this->get_cat_id(), $this->get_input_data_arr());
...
//		$site_category_id = $site_category_id_arr[0];
		...
		$result_xml_xfgmc_tax_category = apply_filters('xfgmc_xml_google_cat_variable_filter', $result_xml_xfgmc_tax_category, $this->get_cat_id(), $product, $offer, $this->get_feed_id());
...
//		$site_category_id = $site_category_id_arr[0];
		...
		if ($xfgmc_product_type === 'enabled') {
			...
			$product_type_res = apply_filters('xfgmc_product_type_res_variable_filter', $product_type_res, $this->get_cat_id(), $product, $offer, $this->get_feed_id());

//			$product_type_res = apply_filters('xfgmc_product_type_res_variable_filter', $product_type_res, $this->get_cat_id(), $this->get_input_data_arr());
...
//		$site_category_id = $site_category_id_arr[0];
		...
		$result_url = apply_filters('xfgmc_url_filter_var', $result_url, $product, $offer, $this->get_cat_id(), $this->get_feed_id()); /* с версии 2.0.5 */
...
trait XFGMC_Trait_Variable_Get_Image_Link {
	public function get_image_link() {
		...
		$picture_xml = apply_filters('xfgmc_pic_variable_offer_filter', $picture_xml, $product, $this->get_feed_id(), $offer);
...
trait XFGMC_Trait_Variable_Get_Price {
	public function get_price() {
		...
		if ($price_xml == 0 || empty($price_xml)) {
			$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('Price not specified', 'xfgmc'), 'post_id' => $offer->get_id(), 'file' => 'traits-xfgmc-variable.php', 'line' => __LINE__)); return '';
		}
			...
			if ((xfgmc_optionGET('xfgmcp_compare_value', $this->get_feed_id(), 'set_arr') !== false) && (xfgmc_optionGET('xfgmcp_compare_value', $this->get_feed_id(), 'set_arr') !== '')) {
				...
				if ($xfgmcp_compare == '>=') {
					if ($price_xml < $xfgmcp_compare_value) {
						$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('The product price', 'xfgmc').' '. $offer->get_price(). ': < ' . $xfgmcp_compare_value, 'post_id' => $offer->get_id(), 'file' => 'traits-xfgmc-variable.php', 'line' => __LINE__)); return '';
					}
				} else {
					if ($price_xml >= $xfgmcp_compare_value) {
						$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('The product price', 'xfgmc').' '. $offer->get_price(). ': >= ' . $xfgmcp_compare_value, 'post_id' => $offer->get_id(), 'file' => 'traits-xfgmc-variable.php', 'line' => __LINE__)); return '';
		...
		$price_xml = apply_filters('xfgmc_variable_price_xml_filter', $price_xml, $product, $offer, $this->get_feed_id()); /* с версии 2.0.6 */
		...
		if ($xfgmc_sale_price === 'yes') {
			...
			$sale_price = apply_filters('xfgmc_variable_sale_price_xml_filter', $sale_price, $product, $offer, $this->get_feed_id()); /* с версии 2.3.1 */
			...
			if (($sale_price > 0) && ($price_xml === $sale_price)) {
				...
				$result_xml = apply_filters('xfgmc_variable_sale_price_filter', $result_xml, $product, $offer, $this->get_feed_id()); /* с версии 2.0.6 */
			} else {
			...
			if (!empty($size_type) && $size_type !== 'disabled') {
				...
				if (!empty($size_type_xml)) {	
					...
					if (!empty($size_type_xml)) {	
						...
						$size_type_alt = xfgmc_optionGET('xfgmc_gender_alt', $this->get_feed_id(), 'set_arr');
	...
	public function get_size_system() {
		...
		if (!empty($size_system) && $size_system !== 'disabled') {
			...
			if (!empty($size_system_xml)) {	
				...
				$size_system_alt = xfgmc_optionGET('xfgmc_gender_alt', $this->get_feed_id(), 'set_arr');
...
trait XFGMC_Trait_Variable_Get_Shipping_Xml {
	public function get_shipping_xml() {
		...
		if (!empty($weight_xml)) {
			...
			$xfgmc_def_shipping_weight_unit = apply_filters('xfgmc_variable_def_shipping_weight_unit_filter', $xfgmc_def_shipping_weight_unit, $product, $offer, $this->get_feed_id()); /* с версии 2.4.0 */
