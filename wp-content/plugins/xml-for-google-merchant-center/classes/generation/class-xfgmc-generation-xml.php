...
// /			file_put_contents($xfgmc_file_file, $result_xml, FILE_APPEND);
	...
	public function run() {
		...
		switch($status_sborki) {
			case -1: // сборка завершена
				...
				wp_clear_scheduled_hook('xfgmc_cron_sborki', array($this->get_feed_id()));
			break;
			...
			default:
				...
				/*	$status_sborki = -1;
					if ($result === true) {
						xfgmc_optionUPD('xfgmc_status_sborki', $status_sborki, $this->get_feed_id());				
						wp_clear_scheduled_hook('xfgmc_cron_sborki', array($this->get_feed_id())); // останавливаем крон сборки
						new XFGMC_Error_Log('FEED № '.$this->get_feed_id().'; SUCCESS: Сборка успешно завершена; Файл: class-generation-xml.php; Строка: '.__LINE__);
						do_action('xfgmc_after_construct', 'full'); // сборка закончена
					} else {
						new XFGMC_Error_Log('FEED № '.$this->get_feed_id().'; ERROR: На завершающем этапе xfgmc_write_file вернула ошибку! Я не смог записать концовку файла... $result ='.$result.'; Файл: class-generation-xml.php; Строка: '.__LINE__);
	...
	protected function get_feed_header($result_xml = '') {
		...
		xfgmc_optionUPD('xfgmc_date_sborki', $unixtime, $this->get_feed_id(), 'yes', 'set_arr');	
	...
	protected function get_feed_footer($result_xml = '') {
		...
		$unixtime = current_time('Y-m-d H:i'); // время в unix формате 			
		xfgmc_optionUPD('xfgmc_date_sborki_end', $unixtime, $this->get_feed_id(), 'yes', 'set_arr');
