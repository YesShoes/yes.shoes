	...
	public function get_form() { ?>
		<div class="postbox">
			...
			<div class="inside">
				...
				<ul class="xfgmc_ul">
					<li><?php _e('URL your feeds', 'xfgmc'); ?></li>
					<li><?php _e('Files generation status', 'xfgmc'); ?></li>
					<li><?php _e('PHP version information ', 'xfgmc'); ?></li>
					<li><?php _e('Multisite mode status', 'xfgmc'); ?></li>
					<li><?php _e('Technical information and plugin logs', 'xfgmc'); ?> XML for Google Merchant Center</li>
				</ul>
				<p><?php _e('Did my plugin help you upload your products to the', 'xfgmc'); ?> XML for Google Merchant Center?</p>
				<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" enctype="multipart/form-data">
					<p>
						<input type="radio" name="<?php echo $this->get_radio_name(); ?>" value="yes"><?php _e('Yes', 'xfgmc'); ?><br />
						<input type="radio" name="<?php echo $this->get_radio_name(); ?>" value="no"><?php _e('No', 'xfgmc'); ?><br />
					</p>
					...
					<p><textarea rows="6" cols="32" name="<?php echo $this->get_textarea_name(); ?>" placeholder="<?php _e('Enter your text to send me a message (You can write me in Russian or English). I check my email several times a day', 'xfgmc'); ?>"></textarea></p>
					<?php wp_nonce_field($this->get_nonce_action(), $this->get_nonce_field()); ?>
					<input class="button-primary" type="submit" name="<?php echo $this->get_submit_name(); ?>" value="<?php _e('Send data', 'xfgmc'); ?>" />
				</form>	
			</div>
		</div><?php
	}
	...
	public function listen_submits_func() {
		if (isset($_REQUEST[$this->get_submit_name()])) {
			...
			add_action('admin_notices', function() {
				...
				$message = __('The data has been sent. Thank you', 'xfgmc');
	...
	public function send_data() {
		if (!empty($_POST) && check_admin_referer($this->get_nonce_action(), $this->get_nonce_field())) { 	
			...
			if (!class_exists('XmlforGoogleMerchantCenterPro')) {
				...
				if (!defined('xfgmcp_VER')) {define('xfgmcp_VER', 'н/д');} 
