...
if (!function_exists('elessi_yith_woo_product_bundle')) :
 ...
 */
if (!function_exists('elessi_yith_woo_product_bundle_content')) :
...
if (!function_exists('elessi_preprocess_review_images')) :
    function elessi_preprocess_review_images($commentdata) {
        ...
        $files = isset($_FILES['ns_image_upload']) ? $_FILES['ns_image_upload'] : array();
        ...
        if (isset($files['name']) && count($files['name']) > $max_files) {
            $mess = sprintf(esc_html__('Maximum number of files allowed is: %s', 'elessi-theme'), $max_files);
            wp_die($mess);
        }
        ...
        if (isset($files['size']) && !empty($files['size'])) {
            foreach ($files['size'] as $size) {
                if (!$size) {
                    wp_die(esc_html__("File's too large!", 'elessi-theme'));
                }
        ...
        if (isset($files['type']) && !empty($files['type'])) {
            foreach ($files['type'] as $type) {
                ...
                if (!in_array($type_file, array("image/jpg", "image/jpeg", "image/bmp", "image/png", "image/gif"))) {
                    wp_die(esc_html__('Only format accepted: JPG, JPEG, BMP, PNG and GIF', 'elessi-theme'));
                }
            }
        }
